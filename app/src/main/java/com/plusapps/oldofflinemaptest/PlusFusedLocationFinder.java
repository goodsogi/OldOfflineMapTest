...
public class PlusFusedLocationFinder implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public void resetFirstLocationFlag() {
        mIsFirstLocationCatched = true;
    }

    public Location getCurrentLocationFromSharedPreferences() {
        Location location = new Location("current location from sharedpreferences");
        location.setLatitude(getSharePointLat());
        location.setLongitude(getSharePointLng());
        return location;
    }
    ...
    public Location getLastKnownLocation() {
        ...
        if(lastKnownLocation == null) {
            lastKnownLocation = new Location("initial location");

            lastKnownLocation.setLatitude(PPNConstants.KOREA_INITIAL_LOCATION_LAT);
            lastKnownLocation.setLongitude(PPNConstants.KOREA_INITIAL_LOCATION_LNG);
        }
    ...
    private double getSharePointLat() {
        return PlusSharedPreferencesManager.getCurrentLocationLat(mContext);

    }

    private double getSharePointLng() {
        return PlusSharedPreferencesManager.getCurrentLocationLng(mContext);
